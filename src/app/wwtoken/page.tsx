...
export default async function page() {
    ...
    return (
        <div className='grid grid-cols-1 lg:grid-cols-5 gap-20  w-4/5 items-center justify-center m-auto relative'>
            <TokenForm />
            <div
                className='w-full lg:col-span-3 border-2 min-w-80 border-background_alt p-4 rounded-lg flex flex-col gap-3 justify-start'>
                <div className='text-foreground_alt'>
                    You bought{'  '} <span className='text-blue-200 text-xl'>{userTokens.length}</span>{'  '}
                    tokens with total of{'  '}<span className='text-pink-300 text-xl'>{totalCost}</span>
                </div>
                <div className='text-foreground_alt'>
                    Smallest token cost:{'  '}
